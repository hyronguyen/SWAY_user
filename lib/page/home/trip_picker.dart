        ...
        Flexible(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              trailing: IconButton(
  icon: Icon(
    isFavorite(_suggestions[index]['latitude'], _suggestions[index]['longitude'])
        ? Icons.favorite
        : Icons.favorite_border,
    color: isFavorite(_suggestions[index]['latitude'], _suggestions[index]['longitude'])
        ? Colors.red
        : Colors.white,
  ),
onPressed: () async {
  debugPrint("Bấm vào yêu thích: ${_suggestions[index]['place_name']}");

  double lat = _suggestions[index]['latitude'];
  double lng = _suggestions[index]['longitude'];

  var favoriteItem = _favoriteLocations.firstWhere(
  ...
  if (favoriteItem.isNotEmpty) {
    ...
    setState(() {
      _favoriteLocations.remove(favoriteItem);
    });

    // Nếu có ID thì gọi API xóa
    if (favoriteItem.containsKey("id")) {
      _removeFavorite(favoriteItem["id"]);
    }
  } else {
    // 💡 Cập nhật UI ngay lập tức
    setState(() {
