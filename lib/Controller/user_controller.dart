...
class UserController {
  ...
  Future<http.Response> apiGetInformationCustomer(
      int customerId, String token) async {
    final String url =
  ...
  Future<http.Response> apiCheckEmailAndPhone(User user) async {
    const String url =
        "http://10.0.2.2:8080/api/usermanagement/checkEmailAndPhoneExistence"; // URL đăng ký
    final response = await http.post(
      Uri.parse(url),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode(user.toJson()),
  ...
  Future<bool> updateCustomerInfo(String customerId, String name, String phone,
      String birthday, String gender, String token) async {
    print(
        "✅ DEBUG: Token truyền vào API = $token"); // Kiểm tra token có bị lỗi không

    final response = await http.put(
      Uri.parse(
          "http://10.0.2.2:8080/api/usermanagement/update-personal-info?customer_id=$customerId"),
      headers: {
  ...
  Future<Map<String, dynamic>> apiChangePassword(
      ...
      String token) async {
    const String url =
        "http://10.0.2.2:8080/api/usermanagement/change-password";

    try {
      // ✅ Chắc chắn customer_id được gửi dưới dạng String
      final response = await http.put(
        Uri.parse(
            "$url?customer_id=${Uri.encodeComponent(customerId)}"), // 👈 Encode String để tránh lỗi URL
        headers: {
  ...
  Future<bool> rateDriver({
    ...
    const String url = "http://10.0.2.2:8080/api/DriverRating/rate-driver";
    ...
    final body = {
      ...
      "trip_id": tripId,
      "rating": rating,
      "review": review ?? "", // nếu không nhập review thì để chuỗi rỗng
    };
    ...
    print("👉 [rateDriver] status: ${response.statusCode}");
    print("👉 [rateDriver] body: ${response.body}");
